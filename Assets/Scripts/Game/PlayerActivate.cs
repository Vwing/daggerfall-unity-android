            ...
            if (InputManager.Instance.ActionComplete(InputManager.Actions.ActivateCenterObject))
            {
                ...
                if (GameManager.Instance.PlayerMouseLook.cursorActive)
                {
                    if (DaggerfallUnity.Settings.RetroRenderingMode > 0)
                    {
                        ...
                        float xm = mousePosition.x / AScreen.width;
                        float ym = (mousePosition.y - largeHUDHeight) / (AScreen.height - largeHUDHeight);
                    ...
                    else
                    {
                        ...
                        ray = mainCamera.ScreenPointToRay(InputManager.Instance.MousePosition);
                    }
                }
                else
                {
